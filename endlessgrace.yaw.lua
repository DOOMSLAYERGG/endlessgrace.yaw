...
local var_0_42 = LPH_JIT(function(arg_8_0, arg_8_1)
	...
	if arg_8_1 then
		...
		assert(var_8_0, "MAYBE DOWNLOAD" .. arg_8_0 .. ".lua?")
	end

	return false
end)

if not var_0_2 or not var_0_42("gamesense/inspect", true) then
	local function var_0_43()
		return
	end
end

local var_0_44 = var_0_42("pui")
...
local function var_0_119()
	if not var_0_36.get_prop(var_0_113.weapon, "m_iItemDefinitionIndex") then return end
...
LPH_NO_VIRTUALIZE(function()
			...
			general = {
				...
				manual = var_155_4.feature(var_155_5.angles:checkbox("Manual yaw"), function()
					return {
						...
						forward = var_155_5.angles:hotkey("\f<p>forward \f<silent>HK\r", false, 0),
				...
				var_155_4.space(var_155_5.angles),
				var_155_4.header(var_155_5.angles, "Flick Settings"),
				flick = var_155_4.feature(var_155_5.angles:hotkey("Fake flick", true, 0), function()
					return {
						pitch = var_155_5.angles:combobox("\f<p>Pitch\f<z>flp", {
							"Off",
							"Static",
							"Jitter",
							"Spin",
						...
						static = var_155_5.angles:slider("\f<p>Angle\f<z>fls", -89, 89, 0, true, "°", 1),
						ang1 = var_155_5.angles:slider("\f<p>Angle 1\f<z>fl1", -89, 89, 0, true, "°", 1),
						ang2 = var_155_5.angles:slider("\f<p>Angle 2\f<z>fl2", -89, 89, 0, true, "°", 1),
						speed = var_155_5.angles:slider("\f<p>Angle Speed\f<z>fsp", -50, 50, 20, true, " ", 0.1),
						jspeed = var_155_5.angles:slider("\f<p>Speed Ticks\f<z>fjt", 2, 14, 2, true, "t", 1, {[2] = " "}),
						rspeed = var_155_5.angles:slider("\f<p>Speed\f<z>frs", 0, 10, 0, true, " ", 0.1)
					}, true
				end)
			},
			state = {
	...
	for iter_155_0, iter_155_1 in var_0_9(var_0_108.states) do
		...
		var_155_11.off = var_155_6({
			var_155_8,
			"off"
		}, var_155_12:slider("Offset\f<z>", -180, 180, 0, true, "°", 1))
		...
		var_155_11.mod = var_155_4.feature(var_155_6({
			...
			return {
				ways = var_155_6({
					...
					"mod",
					"ways"
				}, var_155_12:slider("\f<p>Ways\f<z>", 3, 45, 3)):depend({
					arg_178_0,
					"Ways",
					"Skitter"
				}),
				deg = var_155_6({
					...
					"mod",
					"deg"
				}, var_155_12:slider("\f<p>Degree\f<z>", -180, 180, 0, true, "°", 1))
			}, function(arg_179_0)
		...
		var_155_11.freeze = var_155_4.feature(var_155_6({
			...
			"freeze",
			"on"
		}, var_155_12:checkbox("Freeze\f<z>")), function()
			return {
				chance = var_155_6({
					var_155_8,
					"freeze",
					"chance"
				}, var_155_12:slider("\f<p>Freeze chance\f<z>frz", 0, 100, 0, true, "%")),
				time = var_155_6({
					var_155_8,
					"freeze",
					"time"
				}, var_155_12:slider("\f<p>Freeze time\f<z>frz", 1, 100, 14, true, "t")),
				cooldown = var_155_6({
					var_155_8,
					"freeze",
					"cooldown"
	...
	do
		...
		for iter_dh_0, iter_dh_1 in var_0_9(var_dh_list) do
			...
			if var_dh_en then
				...
				var_dh_d[var_dh_id .. "_mode"]:depend({ var_dh_d.tab, iter_dh_1 }, { var_dh_en, true })
				var_dh_d[var_dh_id .. "_close"]:depend({ var_dh_d.tab, iter_dh_1 }, { var_dh_en, true })
				var_dh_d[var_dh_id .. "_far"]:depend({ var_dh_d.tab, iter_dh_1 }, { var_dh_en, true })
			else
		...
		var_155_17.yaw = var_155_4.feature(var_155_13({
			...
			return {
				...
				ang = var_155_13({
					...
					"y",
					"ang"
				}, var_155_18:slider("\f<p>Angle\f<z>y", -720, 360, 180, true, "°")):depend({
					var_185_0,
					"Static",
					"Jitter",
					"Random",
	...
	var_0_78.paint_ui:set(function()
		...
		if var_0_67 > 1 then
			var_197_1[#var_197_1 + 1] = var_0_32.format("\f<silent> custom — %s", var_0_65.build)
		end
...
LPH_NO_VIRTUALIZE(function()
	...
	function var_pf.names()
		...
		if var_pf_0 then
			for iter_pf_0 in var_0_32.gmatch(var_pf_0, "[^\r\n]+") do
				var_pf_1[#var_pf_1 + 1] = iter_pf_0
			end
		end
	...
	function var_pf.add(arg_pf_0, arg_pf_1)
		...
		if not var_0_30.has(var_pf_0, arg_pf_0) then
			var_pf_0[#var_pf_0 + 1] = arg_pf_0

			var_pf.write_index(var_pf_0)
		end
	end

	function var_pf.remove(arg_pf_0)
		local var_pf_0 = var_pf.names()
		local var_pf_1 = var_0_30.find(var_pf_0, arg_pf_0)

		if var_pf_1 then
			var_0_30.remove(var_pf_0, var_pf_1)
			var_pf.write_index(var_pf_0)
		end
	...
	function var_198_1.save(arg_200_0, arg_200_1)
		...
		var_pf.add(arg_200_0, var_200_6)
	...
	function var_198_1.import()
		...
		var_pf.add(var_204_1, var_204_4)
		...
		angles = {
			manual_buttons = {
				...
				},
				{
					"forward",
					yaw = -15,
					item = var_0_127.antiaim.general.manual.forward
				},
				{
					"reset",
			...
			work = function(arg_230_0)
				...
				local var_230_7 = not var_230_6 and not var_230_5 and var_0_107.rage.other.peek.hotkey:get()

				var_0_107.aa.angles.freestand:override(var_0_107.rage.other.peek.hotkey:get())
		...
		desync = {
			work = function(arg_238_0)
				...
				if var_238_1 == "Static" then
					var_238_2 = false
				elseif var_238_1 == "Jitter" then
					var_238_2 = var_221_2.switch
				elseif var_238_1 == "Opposite" then
					local var_238_3 = var_238_0.j and var_221_2.switch or not var_238_0.j and var_0_127.antiaim.general.invert:get()

					var_238_2 = not var_238_3
				elseif var_238_0.j then
					var_238_2 = var_221_2.switch
				else
	...
	local function var_221_20(arg_276_0)
		if arg_276_0 <= var_221_19 or var_0_113.exploit.active == var_0_109.exploit.OFF then
			if var_221_2.send_packet then
				local var_276_cfg = var_221_1.cur and var_221_1.cur.freeze

				if var_221_freeze.frozen then
					if var_0_38.tickcount() >= var_221_freeze.until_tick then
						var_221_freeze.frozen = false
					else
						var_221_19 = 0
				...
				if var_276_cfg and var_276_cfg.on and var_276_cfg.chance > 0 and var_276_cfg.time > 0 then
					...
					local var_276_cd = var_276_cfg.cooldown or 0

					if var_276_cd <= 0 or var_276_tc - var_221_freeze.last_tick >= var_276_cd then
						if var_0_34.random_int(1, 10000) <= var_276_cfg.chance * 100 then
							var_221_freeze.frozen = true
							var_221_freeze.until_tick = var_276_tc + var_276_cfg.time
							var_221_freeze.last_tick = var_276_tc
						end
					end
				end
			end
	...
	var_282_13.items = {
		...
		},
		{
			...
			function(arg_367_0, arg_367_1, arg_367_2)
				local var_367_0 = var_0_107.aa.angles.freestand.value and var_0_107.rage.other.peek.hotkey:get()
				local var_367_1 = "FS"
